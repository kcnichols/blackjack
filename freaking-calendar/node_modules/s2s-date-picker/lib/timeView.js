...
var _s2sButton = require('s2s-button');

var _s2sButton2 = _interopRequireDefault(_s2sButton);
...
var TimeView = function (_S2SBaseComponent) {
  _inherits(TimeView, _S2SBaseComponent);
  ...
  function TimeView(props) {
    ...
    _this.displayName = 'TimeView';

    // Initialize state with incoming props for hour and minute.
    _this.state = {
      inputWithFocus: "hour",
      hourValue: _this.props.hour,
      minuteValue: _this.props.minute,
    ...
    _this.ampmClick = _this.ampmClick.bind(_this);
    _moment2.default.locale(_this.props.locale);
    _this.changeTimer = 0;

    _this.hourOnFocus = _this.hourOnFocus.bind(_this);
    _this.minuteOnFocus = _this.minuteOnFocus.bind(_this);
    ...
    _this.backOnClick = _this.backOnClick.bind(_this);
  ...
  _createClass(TimeView, [{
    ...
    value: function componentWillReceiveProps(nextProps) {
      if (nextProps.hasOwnProperty('hour')) {
        this.setState(_extends({}, this.state, { hourValue: nextProps.hour }));
      }

      if (nextProps.hasOwnProperty('minute')) {
        this.setState(_extends({}, this.state, { minuteValue: nextProps.minute }));
      }

      if (nextProps.hasOwnProperty('ampm')) {
        this.setState(_extends({}, this.state, { ampm: nextProps.ampm }));
      }
    }
  }, {
    ...
    key: 'backOnClick',
    value: function backOnClick() {
      if (this.state.inputWithFocus == 'hour') {
        this.setState(_extends({}, this.state, { hourValue: this.state.hourValue.substr(0, this.state.hourValue.length - 1) }));
      } else {
        this.setState(_extends({}, this.state, { minuteValue: this.state.minuteValue.substr(0, this.state.minuteValue.length - 1) }));
      }
    }
  }, {
    key: 'getButtons',
    value: function getButtons() {
      ...
      var buttons = ['1', '2', '3', '4', '5', '6', '7', '8', '9', '0'].map(function (button, i) {
        return _react2.default.createElement(
          'div',
          {
            style: button !== '0' ? _this2.button : _extends({}, _this2.button, { width: '60px', flex: '4 0 43%', marginLeft: '5px' }),
            key: i,
            role: 'buttons',
            tabIndex: 0,
      ...
      return _react2.default.createElement(
        ...
        buttons,
        _react2.default.createElement(_s2sButton2.default, {
    ...
    key: 'isTextSelected',
    value: function isTextSelected(input) {
    ...
    value: function handleInputChange(event) {
      ...
      if (newValue.length == 2 && this.state.inputWithFocus == 'hour') {
        ...
        newState = _extends({}, newState, { inputWithFocus: 'minute' });
      }
      ...
      this.changeTimer = setTimeout(function () {
        ...
        var paddedValue = '0' + newValue;
        paddedValue = paddedValue.substring(paddedValue.length - 2);
        // If padding is needed we set the state.
        if (paddedValue !== _this3.state[valueToUpdate]) {
          newState[valueToUpdate] = paddedValue;
          _this3.setState(_extends({}, _this3.state, newState));
        }

        // Validating numbers are between 1-12 in 12 hour format
        var doSendData = true;
        if (_this3.props.isTwelveHour && _this3.state.hourValue > 12) {
          console.error(_this3.state.hourValue + ' is not a valid value for hour');
          doSendData = false;
        }
        // Validating numbers are between 0-23in 24 hour format
        if (!_this3.props.isTwelveHour && _this3.state.hourValue > 23) {
          console.error(_this3.state.hourValue + ' is not a valid value for hour');
          doSendData = false;
        }
        // Validating numbers are between 0-59 minutes format
        if (_this3.state.minuteValue > 59) {
          console.error(_this3.state.minuteValue + ' is not a valid value for minute');
          doSendData = false;
        }

        if (doSendData == true) {
          _this3.props.cbInputChange(_this3.state.hourValue, _this3.state.minuteValue, _this3.state.ampm);
        } else {
    ...
    value: function ampmClick() {
      ...
      this.setState(_extends({}, this.state, { ampm: ampm }));
      if (this.changeTimer != 0) {
        clearTimeout(this.changeTimer);
      }
      this.changeTimer = setTimeout(function () {
        _this4.props.cbInputChange(_this4.state.hourValue, _this4.state.minuteValue, _this4.state.ampm);
      }, 1000);
    }
  }, {
    key: 'minuteOnFocus',
    value: function minuteOnFocus() {
      this.refs.minute.select();
      this.setState(_extends({}, this.state, { inputWithFocus: 'minute' }));
    }
  }, {
    key: 'hourOnFocus',
    value: function hourOnFocus() {
      //console.log('minute focused');
      this.refs.hour.select();
      this.isKeyClickConcat = false;
      this.setState(_extends({}, this.state, { inputWithFocus: 'hour', hourSelected: true }));
    }
  }, {
    key: 'render',
    value: function render() {
      ...
      return _react2.default.createElement(
        ...
        _react2.default.createElement(
          ...
          _react2.default.createElement(
            'div',
            { style: this.topContainerTimeView },
            _react2.default.createElement(
              'div',
              {
            ...
            this.props.isTwelveHour === false ? undefined : _react2.default.createElement(_s2sButton2.default, {
              ariaLabel: 'date time',
              className: 'am-pm',
              placeholder: 'ampm',
              ref: 'ampm',
        ...
        _react2.default.createElement(
          'div',
          { style: this.buttonWrapperStyles },
          this.getButtons()
        )
      );
    }
...
TimeView.propTypes = {
  ampm: _react2.default.PropTypes.string,
  cbInputChange: _react2.default.PropTypes.func,
  compStyle: _react2.default.PropTypes.object,
  hour: _react2.default.PropTypes.string,
  isTwelveHour: _react2.default.PropTypes.bool,
  locale: _react2.default.PropTypes.string,
  minute: _react2.default.PropTypes.string
};
TimeView.defaultProps = {
  ampm: 'am',
  cbInputChange: function cbInputChange() {
    console.log('Please provide a callback function for cbInputChange');
  },
  isTwelveHour: true,
  hour: '12',
  locale: 'en',
