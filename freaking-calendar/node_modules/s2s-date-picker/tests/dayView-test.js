...
import DayView from '../src/dayView.js';
...
describe('DayView Component', ()=>{
  ...
  it('rerenders when props have changed', () => {
    const spy = sinon.spy(DayView.prototype, "render");
  ...
  it('renders date value', () => {
    ...
    //console.log('wrapper', wrapper.find('ViewHeader').childAt(1).find('span').debug());
    expect(wrapper.find('ViewHeader').childAt(1).find('span').text()).toEqual('January 1970');
  });

  it('fires cellClick function on click', () => {
    const spy1 = sinon.spy(DayView.prototype, 'cellClick')
    const wrapper = mount(
      <DayView
          dateVal={ddate1}
      />);
  ...
  it('fires cellClick function on keypress', () => {
    const spy1 = sinon.spy(DayView.prototype, 'daysOnKeyPress');
    const wrapper = mount(
      <DayView
          dateVal={ddate1}
      />);

    wrapper.childAt(1).simulate('keydown');
    expect(spy1.calledOnce).toEqual(true);
  });

  it('fires prev function', () => {
    const spy1 = sinon.spy(DayView.prototype, 'prev');
  ...
  it('fires handleTitleClick function', () => {
    const spy2 = sinon.spy(DayView.prototype, 'handleTitleClick');
  ...
  it('fires next function', () => {
    const spy1 = sinon.spy();
    const spy2 = sinon.spy(DayView.prototype, 'next');
    const wrapper = mount(
      <DayView
          dateVal={ddate1}
          cbSetDate={spy1}
